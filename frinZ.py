...
help_cpu       = "FFT/IFFT を実行する際に使用する CPU の数"
...
parser.add_argument("--delay-window"    , default=False      , dest="del_win"  , help=help_del_win )
parser.add_argument("--rate-window"     , default=False      , dest="rate_win" , help=help_rate_win)
...
if cor_header_file_exist == False :
    ...
    Source
        Name = {source_name}
        Position (RA, Decl) = ({source_position_ra},{source_position_dec}) deg, J2000
    """ 
...
if (delay_win != False and rate_win == False) or (delay_win == False and rate_win != False):
    ...
    exit()
if delay_win != False :
    delay_window_low, delay_window_high = delay_win.split("[")[-1].split("]")[0].split(",")
    ...
    if float(delay_window_low) > float(delay_window_high) :
        ...
        exit()
if rate_win != False :
    rate_window_low, rate_window_high = rate_win.split("[")[-1].split("]")[0].split(",")
