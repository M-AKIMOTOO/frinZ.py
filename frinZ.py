...
for l in range(loop) :
    ...
    if freq_plot != True and time_plot :
        ...
        ax4.text(-0.25,0.50,"Peak Amp [%]", fontsize=13.5)                   ; ax4.text(1.05,0.50, "%.6f" % (fringe_lag_rate_00_amp * 100), fontsize=13.5, ha='right')
        ax4.text(-0.25,0.40,"Peak Phs [deg]", fontsize=13.5)                 ; ax4.text(1.05,0.40, "%+.5f" % fringe_lag_rate_00_phase, fontsize=13.5, ha='right')
        ax4.text(-0.25,0.30,"SNR (1 \u03C3 [%])", fontsize=13.5)             ; ax4.text(1.05,0.30, "%.3f (%.6f)" % (SNR_time_lag, noise_level_lag*100), fontsize=13.5, ha='right')
